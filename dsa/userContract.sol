...
contract userContract is Initializable {
    ...
    mapping(uint256 => mapping(address => int256)) public withdrawAmt;
    mapping(uint256 => bool) public isWithdraw;
    mapping(uint256 => address) public orderIdWithdrawToken;
    ...
    function executeOrderSimilarDexUniswapV2(
        ...
        orderIdWithdrawToken[orderId] = path[1];
        withdrawAmt[orderId][path[1]] = int(amount[1]);
        isWithdraw[orderId] = true;
    ...
    function executeOrderUniswapV3(
        ...
        orderIdWithdrawToken[orderId] = params.tokenOut;
        withdrawAmt[orderId][params.tokenOut] = int(amount);
        isWithdraw[orderId] = true;
    }
    ...
    function executeOrderBalancerV2(
        ...
        orderIdWithdrawToken[orderId] = singleSwap.tokenOut;
        withdrawAmt[orderId][singleSwap.tokenOut] = int(amount);
        isWithdraw[orderId] = true;
    }

    function withdraw(uint256 orderId) external onlyOwner {
        require(!isCanceled[orderId], "order is cancelled");
        address erc20 = orderIdWithdrawToken[orderId];
        int256 amt = withdrawAmt[orderId][erc20];
        bool _true = isWithdraw[orderId];
        require(_true, "not a valid orderId for withdraw");
        isWithdraw[orderId] = false;
        withdrawAmt[orderId][erc20] = 0;
        ERC20Interface(erc20).transfer(msg.sender, uint256(amt));
    }
