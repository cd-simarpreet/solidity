...
import { AaveInterface } from "./interface2.sol";

abstract contract AaveResolver is Events, Helpers {
    uint256 constant  maxValue = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;
	function deposit(
		address token,
		uint256 amt,
		uint256 getId,
		...
		returns (string memory _eventName, bytes memory _eventParam)
	{
		...
		// AaveInterface aave = AaveInterface(0x082307FbDEe3967141E96f9db16c4ea1f206F5A2);
		...
		if (isEth) {
			_amt = _amt == maxValue ? address(this).balance : _amt;
	 ...
	 * @param amt The amount of the token to withdraw. (For max: `maxValue`)
	 ...
	 * @param amt The amount of the token to payback. (For max: `maxValue`)
