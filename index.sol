...
/**
 * @title Index
 * @dev Registry Contract for Storage of user and accounts.
 */
...
contract Variables is DSMath {
    ...
    address immutable MinimalProxy;

    constructor(address _MinimalProxy) {
...
contract Configure is Variables {
    ...
    constructor (address _MinimalProxy) Variables(_MinimalProxy) {
    }
    /**
    ...
    */
    function addAccount(address _owner, uint64 _account) external {
        // user already craeted Smart Account.
        ...
        // user is creating Smart account first time.
     ...
     * @param _account Smart Account Address.
    */
    function removeAccount(address _owner, uint64 _account) external {
     ...
     * @param _account Smart Account Address.
    // */
    // function addUser(address _owner, uint64 _account) external {
     ...
     * @param _account Smart Account Address.
    */
    // function removeUser(address _owner, uint64 _account) external {
...
contract Index is Configure {
    constructor (address _MinimalProxy) Configure(_MinimalProxy) {}

    function init(address  _account) external returns(uint64) {
        require(msg.sender == MinimalProxy, "not-minimal Proxy!!");
        accounts++;
